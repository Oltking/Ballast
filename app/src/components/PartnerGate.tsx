...
export default function PartnerGate({ state }: { state: VaultState }) {
  ...
  return (
    <div className="panel">
      ...
      <div className={`verdict ${accepts ? "solvent" : "bad"}`}>
        <div className="dot" />
        <div>
          ...
          <div className="note">
            {accepts
              ...
              : "require_fresh_attestation reverts — no fresh solvent credential (stale, insolvent, or wind-down). The partner contract's transaction fails."}
          </div>
        </div>
      </div>
    </div>
